...
namespace LI2Sup {
            ...
            [&](const uint& i) {
                if ((msg->points[i].tag & 0x30) == 0x10 ||
                    ...
                    // if (i % g_filter_rate == 0)
                    {
                        ...
                        cloud_full->at(i).intensity =
                            msg->points[i].reflectivity;

                        if ((abs(cloud_full->at(i).x -
                                 cloud_full->at(i - 1).x) > 1e-7) ||
                            (abs(cloud_full->at(i).y -
                                 cloud_full->at(i - 1).y) > 1e-7) ||
                            (abs(cloud_full->at(i).z -
                                 cloud_full->at(i - 1).z) > 1e-7)) {
                            double normal_dis =
                                cloud_full->at(i).x * cloud_full->at(i).x +
                                cloud_full->at(i).y * cloud_full->at(i).y +
                                cloud_full->at(i).z * cloud_full->at(i).z;
                            if (normal_dis > g_blind2 and
                                normal_dis < g_maxrange2) {
                                is_valid_pt[i] = true;
                            }
                        }
                    }
    ...
    void ROSWrapper::setupIO()
    {
        ...
        sub_imu_ = this->create_subscription<sensor_msgs::msg::Imu>(g_imu_topic,
            imu_qos,
        ...
        if (g_lidar_type == LID_TYPE::LIVOX) {
            sub_lidar_ =
                this->create_subscription<livox_ros_driver2::msg::CustomMsg>(
                    g_lidar_topic,
                    lidar_qos,
                    std::bind(&ROSWrapper::livoxHandler,
                        this,
                        std::placeholders::_1),
                    sub_opt);
        }
            ...
            sub_lidar_std_ =
                this->create_subscription<sensor_msgs::msg::PointCloud2>(
                    g_lidar_topic,
                    lidar_qos,
                    std::bind(&ROSWrapper::stdMsgHandler,
                        this,
                        std::placeholders::_1),
                    sub_opt);
        }
        ...
        /// output ======================================
        pub_odom_ =
            this->create_publisher<nav_msgs::msg::Odometry>("/lio/odom", 100);

        pub_imu_odom_ =
            this->create_publisher<nav_msgs::msg::Odometry>("/lio/imu/odom",
                10);

        pub_robo_odom_ =
            this->create_publisher<nav_msgs::msg::Odometry>("/lio/robo/odom",
                10);

        pub_path_ =
            this->create_publisher<nav_msgs::msg::Path>("/lio/path", 10);
        ...
        for (std::size_t _i = 0; _i < ptsize; _i += g_filter_rate) {
            ...
            if (tag == 0x10 || tag == 0x00) {
                ...
                if (dis > g_blind2 && dis < g_maxrange2) {
                    ...
                    lidar_data.pc->emplace_back(pt.x,
                        pt.y,
                        pt.z,
                        pt.reflectivity,
                        offset_time);
        ...
        const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
        ...
        switch (g_lidar_type) {
            ...
            case LID_TYPE::HESAI16: {
                ...
                for (std::size_t i = 0; i < pl_orig.size();
                     i += g_filter_rate) {
    ...
    void ROSWrapper::pub_odom(const NavState& state)
    {
        ...
        // 发布 world -> base_link 的动态TF变换 (经过 odom_robo 变换)
        tf_msg.header.stamp = odom.header.stamp;
        tf_msg.header.frame_id = "world";
        tf_msg.child_frame_id = "base_link";

        // 使用经过变换后的机器人位置和姿态
        tf_msg.transform.translation.x = robo_position[0];
        tf_msg.transform.translation.y = robo_position[1];
        tf_msg.transform.translation.z = robo_position[2];

        tf_msg.transform.rotation.x = robo_temp_q.x();
        tf_msg.transform.rotation.y = robo_temp_q.y();
        tf_msg.transform.rotation.z = robo_temp_q.z();
        tf_msg.transform.rotation.w = robo_temp_q.w();

        tf_broadcaster_->sendTransform(tf_msg);


        //LOG(INFO)<< GREEN << "pub world -> imu tf" << RESET;
        ...
        bool flg_finish_init)
    {
        static auto init_pose_sub = this->create_subscription<
            ...
            [this, &init_pose, &flg_get_init_guess](
                ...
                init_pose = BASIC::SE3(SO3(init_rotation.toRotationMatrix()),
                    init_translation);

                flg_get_init_guess = true;

                LOG(INFO) << YELLOW << " ---> GET Initial guess: "
                          << init_translation.transpose() << " yaw: "
