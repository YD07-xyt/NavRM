...
AimDriver::AimDriver(const rclcpp::NodeOptions &options)
  ...
  // 2. 使用完整的命名空间
  send_pub_ = this->create_publisher<aim_driver::msg::Control>("/sentry_to_aim_data", 10);
}
...
void AimDriver::send_data(const geometry_msgs::msg::Twist::SharedPtr msg) {
  ...
  try {
    ...
    RCLCPP_ERROR(get_logger(), "Failed to process and publish control: %s",
                 e.what());
  } catch (...) {
