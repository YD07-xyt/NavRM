...
namespace planner::grid_env {
        ...
        grid_map::GridMap init_grid_map(const GridMapConfig& grid_map_config)
        {
            ...
            grid_map.setGeometry(grid_map::Length(grid_map_config.map_size_x,
                ...
                grid_map::Position(grid_map_config.map_postion_x,
                    grid_map_config.map_postion_y));// 地图尺寸、分辨率、原点
            ...
            grid_map::GridMap& grid_map)
        {
            ...
            grid_map::Matrix& elevation_layer = grid_map["elevation"];
            ...
            for (const auto& point: point_cloud->points) {
                ...
                if (grid_map.isInside(pos)) {
                    ...
                    if (grid_map.getIndex(pos, index)) {
                        // 使用索引访问，更快
                        elevation_layer(index.x(), index.y()) = point.z;
                        inside_count++;
                    }
                }
                else {
            ...
            spdlog::info("高程层初始化完成: 有效点={}, 外部点={}, NaN点={}",
                inside_count,
                outside_count,
                nan_count);
        }
            ...
            const float max_obstacle_height)
        {
            ...
            grid_map::Index index;
            grid_map::Matrix& obstacle_layer = grid_map["obstacles"];
            for (const auto& point: point_cloud->points) {
                if (point.z > min_obstacle_height &&
                    ...
                    if (grid_map.getIndex(position, index)) {
                        if (index.x() >= 0 &&
                            index.x() < obstacle_layer.rows() &&
                            index.y() >= 0 &&
                            index.y() < obstacle_layer.cols()) {
                            obstacle_layer(index.x(), index.y()) = 1.0;
                        }
                        else {
                            spdlog::warn("索引越界: ({}, {})", index.x(), index.y());
                        }
                    }
                }
            }
    ...
    * @return: 输出map
    */
        void init_inflation_map_layer(grid_map::GridMap& grid_map,
            ...
            const std::string& outputLayer)
        {
            ...
            for (int j = 0; j < cols; ++j) {
                for (int i = 1; i < rows; ++i) {
                    esdf(i, j) =
                        std::min<float>(esdf(i, j), esdf(i - 1, j) + res);
                }
            ...
            for (int i = 0; i < rows; ++i) {
                for (int j = 1; j < cols; ++j) {
                    esdf(i, j) =
                        std::min<float>(esdf(i, j), esdf(i, j - 1) + res);
                }
