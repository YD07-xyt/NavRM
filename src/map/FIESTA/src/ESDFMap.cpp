...
fiesta::ESDFMap::ESDFMap(Eigen::Vector3d origin,
    double resolution_,
    int reserve_size)
    : origin_(origin)
    , resolution_(resolution_)
{
    ...
    std::fill(distance_buffer_.begin(),
        distance_buffer_.end(),
        (double) undefined_);
    std::fill(occupancy_buffer_.begin(), occupancy_buffer_.end(), 0);
    std::fill(closest_obstacle_.begin(),
        closest_obstacle_.end(),
        Eigen::Vector3i(undefined_, undefined_, undefined_));
    std::fill(vox_buffer_.begin(),
        vox_buffer_.end(),
        Eigen::Vector3i(undefined_, undefined_, undefined_));
...
#ifdef PROBABILISTIC
void fiesta::ESDFMap::SetParameters(double p_hit,
    double p_miss,
    double p_min,
    double p_max,
    double p_occ)
...
bool fiesta::ESDFMap::UpdateOccupancy(bool global_map)
{
    ...
    while (!occupancy_queue_.empty()) {
        ...
        int occupy = static_cast<int>(Exist(idx));
        ...
        occupancy_buffer_[idx] = std::min(
            ...
            clamp_max_log_);
        if (Exist(idx) && (occupy == 0)) {
            insert_queue_.push(QueueElement{xx.point_, 0.0});
        }
        else if (!Exist(idx) && (occupy != 0)) {
...
int fiesta::ESDFMap::GetOccupancy(Eigen::Vector3d pos)
{
    ...
    return static_cast<int>(Exist(Vox2Idx(vox)));
}

int fiesta::ESDFMap::GetOccupancy(Eigen::Vector3i pos_id)
{
    // TODO: no boundary check
    return static_cast<int>(Exist(Vox2Idx(std::move(pos_id))));
}
...
void fiesta::ESDFMap::GetPointCloud(sensor_msgs::msg::PointCloud2& m,
    int vis_lower_bound,
    int vis_upper_bound)
{
...
void fiesta::ESDFMap::GetSliceMarker(visualization_msgs::msg::Marker& m,
    int slice,
    int id,
...
void fiesta::ESDFMap::IncreaseCapacity(int& old_size, int new_size)
{
    ...
    std::fill(distance_buffer_.begin() + old_size,
        distance_buffer_.end(),
        (double) undefined_);
    std::fill(occupancy_buffer_.begin() + old_size, occupancy_buffer_.end(), 0);
    std::fill(closest_obstacle_.begin() + old_size,
        closest_obstacle_.end(),
        Eigen::Vector3i(undefined_, undefined_, undefined_));
    std::fill(vox_buffer_.begin() + old_size,
        vox_buffer_.end(),
        Eigen::Vector3i(undefined_, undefined_, undefined_));
    ...
    bool new_vec)
{
    ...
    Pos2Vox(max_pos - Eigen::Vector3d(resolution_ / 2,
                          resolution_ / 2,
                          resolution_ / 2),
        max_vec_);
}
...
{
#ifdef HASH_TABLE
    Eigen::Vector3i ma = Eigen::Vector3i(0, 0, 0),
                    mi = Eigen::Vector3i(0, 0, 0);
    ...
    for (int ii = 1; ii < count; ii++) {
        if (distance_buffer_[ii] >= 0 && distance_buffer_[ii] < infinity_) {
            kdtree.nearestKSearch(pcl::PointXYZ(vox_buffer_[ii](0),
                                      vox_buffer_[ii](1),
                                      vox_buffer_[ii](2)),
                1,
                pointIdxNKNSearch,
        ...
        for (int y = 0; y < grid_size_(1); ++y) {
            for (int z = 0; z < grid_size_(2); ++z) {
                ...
                if (distance_buffer_[ii] >= 0 &&
                    ...
                    kdtree.nearestKSearch(pcl::PointXYZ(x, y, z),
                        1,
                        pointIdxNKNSearch,
                        pointNKNSquaredDistance);
                    double tmp = sqrt(pointNKNSquaredDistance[0]) * resolution_;
