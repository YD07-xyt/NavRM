#ifndef ESDF_TOOLS_INCLUDE_FIESTA_H
#define ESDF_TOOLS_INCLUDE_FIESTA_H
#include "ESDFMap.h"
#include "raycast.h"
#include "timing.h"
...
namespace fiesta {
    ...
    private:
...
#endif
        rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr slice_pub;
        ...
        std::queue<
            std::tuple<rclcpp::Time, Eigen::Vector3d, Eigen::Quaterniond>>
            transform_queue_;
...
#endif
        void Visualization(ESDFMap* esdf_map,
            bool global_vis,
    ...
    public:
        explicit Fiesta(rclcpp::Node::SharedPtr node);
        ~Fiesta();
    };

    template <class DepthMsgType, class PoseMsgType>
    Fiesta<DepthMsgType, PoseMsgType>::Fiesta(rclcpp::Node::SharedPtr node)
        : node_(node)
    {
...
#ifdef HASH_TABLE
        esdf_map_ = new ESDFMap(Eigen::Vector3d(0, 0, 0),
            parameters_.resolution_,
            parameters_.reserved_size_);
#ifdef SIGNED_NEEDED
        inv_esdf_map_ = new ESDFMap(Eigen::Vector3d(0, 0, 0),
            parameters_.resolution_,
            parameters_.reserved_size_);
#endif
#else
        esdf_map_ = new ESDFMap(parameters_.l_cornor_,
            parameters_.resolution_,
            parameters_.map_size_);
#ifdef SIGNED_NEEDED
        inv_esdf_map_ = new ESDFMap(parameters_.l_cornor_,
            parameters_.resolution_,
            parameters_.map_size_);
#endif
#endif
...
#ifdef PROBABILISTIC
        esdf_map_->SetParameters(parameters_.p_hit_,
            parameters_.p_miss_,
            parameters_.p_min_,
            parameters_.p_max_,
            parameters_.p_occ_);
        ...
        transform_sub_ =
            node_->create_subscription<geometry_msgs::msg::TransformStamped>(
                "odom",
                10,
                std::bind(&Fiesta::PoseCallback, this, std::placeholders::_1));

        depth_sub_ =
            node_->create_subscription<sensor_msgs::msg::PointCloud2>("/livox/lidar",
                10,
                std::bind(&Fiesta<DepthMsgType, PoseMsgType>::DepthCallback,
                    this,
                    std::placeholders::_1));

        //发布
        slice_pub = node_->create_publisher<visualization_msgs::msg::Marker>(
            "ESDFMap/slice",
            10);
        ...
        //定时器
        update_mesh_timer_ = node->create_wall_timer(
...
#ifdef SIGNED_NEEDED
        if (inv_esdf_map_ != nullptr) {
            delete inv_esdf_map_;
            inv_esdf_map_ = nullptr;
        }
#endif
    }
        ...
        const std::string& text)
    {
        ...
        if (esdf_map != nullptr) {
            ...
            if (global_vis) {
                ...
                esdf_map->SetUpdateRange(cur_pos_ - parameters_.radius_,
            ...
            esdf_map->GetPointCloud(pc,
                parameters_.vis_lower_bound_,
                parameters_.vis_upper_bound_);
            occupancy_pub_->publish(pc);

            visualization_msgs::msg::Marker slice_marker;
            esdf_map->GetSliceMarker(slice_marker,
                parameters_.slice_vis_level_,
                100,
                Eigen::Vector4d(0, 1.0, 0, 1),
        ...
        if (!text.empty()) {
            ...
            marker.header.stamp = this->clock_->now();
            marker.id = 3456;
    ...
    template <class DepthMsgType, class PoseMsgType>
    void
    Fiesta<DepthMsgType, PoseMsgType>::RaycastProcess(int i, int part, int tt)
    {
        ...
        for (int idx = part * i; idx < part * (i + 1); idx++) {
            ...
            Eigen::Vector4d tmp =
                transform_ * Eigen::Vector4d(pt.x, pt.y, pt.z, 1);
            Eigen::Vector3d point =
                Eigen::Vector3d(tmp(0), tmp(1), tmp(2)) / tmp(3);
            ...
            for (int i = output.size() - 2; i >= 0; i--) {
                Eigen::Vector3d tmp =
                    (output[i] + half) * parameters_.resolution_;
    ...
    void Fiesta<DepthMsgType, PoseMsgType>::RaycastMultithread()
    {
        ...
        if (parameters_.ray_cast_num_thread_ == 0) {
            ...
            std::list<std::thread> integration_threads =
                std::list<std::thread>();
            //TODO : size_t i=0 -->  int i=0
                for (int i = 0; i < parameters_.ray_cast_num_thread_; ++i) {
                integration_threads.emplace_back(&Fiesta::RaycastProcess,
                    this,
                    i,
                    part,
                    tt);
    ...
    template <class DepthMsgType, class PoseMsgType>
    double Fiesta<DepthMsgType, PoseMsgType>::GetInterpolation(
        const cv::Mat& img,
        double u,
        double v)
    {
    ...
    void Fiesta<DepthMsgType, PoseMsgType>::DepthConversion()
    {
        ...
        cv::Mat& last_img = img_[(image_cnt_ & 1) == 0u];

        cv_bridge::CvImagePtr cv_ptr;
        cv_ptr = cv_bridge::toCvCopy(depth_queue_.front(),
            depth_queue_.front()->encoding);
        // TODO: make it a parameter
        constexpr double k_depth_scaling_factor = 1000.0;
        if (depth_queue_.front()->encoding ==
            sensor_msgs::image_encodings::TYPE_32FC1) {
            (cv_ptr->image)
                .convertTo(cv_ptr->image, CV_16UC1, k_depth_scaling_factor);
        }
        ...
        if (!parameters_.use_depth_filter_) {
            for (int v = 0; v < rows; v++) {
                ...
                for (int u = 0; u < cols; u++) {
                    ...
                    pcl::PointXYZ point;
                    point.x = (u - parameters_.center_x_) * depth /
                              parameters_.focal_x_;
                    point.y = (v - parameters_.center_y_) * depth /
                              parameters_.focal_y_;
                    point.z = depth;
            ...
            if (image_cnt_ != 1) {
                ...
                for (int v = parameters_.depth_filter_margin_;
                     v < rows - parameters_.depth_filter_margin_;
                     v++) {
                    row_ptr = current_img.ptr<uint16_t>(v) +
                              parameters_.depth_filter_margin_;
                    for (int u = parameters_.depth_filter_margin_;
                         u < cols - parameters_.depth_filter_margin_;
                         u++) {
                        depth = (*row_ptr++) / k_depth_scaling_factor;
                        pcl::PointXYZ point;
                        point.x = (u - parameters_.center_x_) * depth /
                        ...
                        point.y = (v - parameters_.center_y_) * depth /
                                  parameters_.focal_y_;
                        point.z = depth;
                        if (depth > parameters_.depth_filter_max_dist_ ||
                            depth < parameters_.depth_filter_min_dist_) {
                            continue;
                        }
                        coord_h = last_transform_.inverse() * transform_ *
                                  Eigen::Vector4d(point.x, point.y, point.z, 1);
                        coord = Eigen::Vector3d(coord_h(0),
                                    coord_h(1),
                                    coord_h(2)) /
                                coord_h(3);
                        uu = coord.x() * parameters_.focal_x_ / coord.z() +
                             parameters_.center_x_;
                        vv = coord.y() * parameters_.focal_y_ / coord.z() +
                             parameters_.center_y_;
                        if (uu >= 0 && uu < cols && vv >= 0 && vv < rows) {
                            //                        getInterpolation(last_img, uu, vv)
                            if (fabs(last_img.at<uint16_t>((int) vv, (int) uu) /
    ...
    {
        if (esdf_map_ == nullptr) {
            RCLCPP_ERROR(node_->get_logger(), "ESDF map is not initialized!");
            return;
        }
        ...
        while (!depth_queue_.empty()) {
            ...
            while (
                transform_queue_.size() > 1 &&
                std::get<0>(transform_queue_.front()) <=
                    depth_time + rclcpp::Duration::from_seconds(time_delay)) {
            ...
            new_msg_ = true;
            std::cout<<"new_msg_ true"<<std::endl;
#ifndef PROBABILISTIC
            ...
            raycast_origin_ = Eigen::Vector3d(transform_(0, 3),
                                  transform_(1, 3),
                                  transform_(2, 3)) /
                              transform_(3, 3);

            if constexpr (std::is_same<DepthMsgType,
                              sensor_msgs::msg::Image::ConstSharedPtr>::value) {
                DepthConversion();
            }
            else if constexpr (std::is_same<DepthMsgType,
                                   sensor_msgs::msg::PointCloud2::
                                       ConstSharedPtr>::value) {
                sensor_msgs::msg::PointCloud2::ConstSharedPtr tmp =
                    depth_queue_.front();
                pcl::fromROSMsg(*tmp, cloud_);
            }

            std::cout << "Pointcloud Size:\t" << cloud_.points.size()
                      << std::endl;
    ...
    template <typename DepthMsgType, typename PoseMsgType>
    void Fiesta<DepthMsgType, PoseMsgType>::PoseCallback(const PoseMsgType& msg)
    {
        Eigen::Vector3d pos;
        Eigen::Quaterniond q;

        if constexpr (std::is_same<PoseMsgType,
        ...
        else if constexpr (std::is_same<PoseMsgType,
    ...
    void Fiesta<DepthMsgType, PoseMsgType>::DepthCallback(
        const typename DepthMsgType::SharedPtr depth_map)// 改为 SharedPtr
    {
        if (!depth_map) {
            RCLCPP_WARN(node_->get_logger(), "Received null depth message");
            return;
        }
        depth_queue_.push(*depth_map);
        SynchronizationAndProcess();
        std::cout<<"SynchronizationAndProcess from DepthCallback "<<std::endl;
    }
    ...
    void Fiesta<DepthMsgType, PoseMsgType>::UpdateEsdfEvent()
    {
        ...
        std::cout<<"UpdateEsdfEvent start"<<std::endl;
        new_msg_ = false;
        cur_pos_ = sync_pos_;
        ...
        if (parameters_.visualize_every_n_updates_ != 0 &&
            ...
            std::cout<<"Visualization process"<<std::endl;
