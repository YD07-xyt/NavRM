...
if [ $ROS_VERSION = ${VERSION_ROS1} ]; then
    ...
    cd ../../
    colcon build --cmake-args -DROS_EDITION=${VERSION_ROS2} -DDISTRO_ROS=${ROS_DISTRO} -DCMAKE_BUILD_TYPE=Release  -DCMAKE_EXPORT_COMPILE_COMMANDS=ON --symlink-install --parallel-workers 4 
fi
popd > /dev/null
