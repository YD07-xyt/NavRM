...
namespace rc_esdf {
    ...
    void RcEsdfMap::visualizeEsdf(const std::vector<Eigen::Vector2d>& footprint)
    {
        ...
        for (int y = 0; y < grid_size_y_; ++y) {
            for (int x = 0; x < grid_size_x_; ++x) {
                ...
                if (val < -1e-4) {
                    ...
                    uchar r = cv::saturate_cast<uchar>(
                        (std::abs(val) / max_inner) * 255.0f);
                    color = cv::Vec3b(0, 0, r);
                }
                else if (val > 1e-4) {
                    // 外部使用 max_outer 归一化
                    uchar g =
                        cv::saturate_cast<uchar>((val / max_outer) * 255.0f);
        ...
        auto worldToImg = [&](const Eigen::Vector2d& w_pos) {
            ...
            return cv::Point(static_cast<int>(gx * scale),
                static_cast<int>((grid_size_y_ - gy) * scale));
        };

        // --- 4. 绘制坐标轴 (深蓝色) ---
        cv::Point origin_pix = worldToImg(Eigen::Vector2d(0, 0));
        cv::line(img,
            cv::Point(origin_pix.x, 0),
            cv::Point(origin_pix.x, h_img),
            cv::Scalar(255, 50, 0),
            1);
        cv::line(img,
            cv::Point(0, origin_pix.y),
            cv::Point(w_img, origin_pix.y),
            cv::Scalar(255, 50, 0),
            1);
        ...
        if (!footprint.empty()) {
            for (size_t i = 0; i < footprint.size(); ++i) {
                ...
                cv::Point p2 =
                    worldToImg(footprint[(i + 1) % footprint.size()]);
        ...
        for (int y = 0; y < grid_size_y_; y += 2) {
            for (int x = 0; x < grid_size_x_; x += 2) {
                double dist;
                Eigen::Vector2d grad;
                Eigen::Vector2d p_world(origin_x_ + (x + 0.5) * resolution_,
                    origin_y_ + (y + 0.5) * resolution_);
                if (query(p_world, dist, grad) && grad.norm() > 0.05) {
                    cv::Point p_start = worldToImg(p_world);
                    cv::Point p_end(p_start.x +
                                        static_cast<int>(grad.x() * scale),
