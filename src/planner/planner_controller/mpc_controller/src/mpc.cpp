...
MpcController::MpcController(rclcpp::Node::SharedPtr node): node_(node){

    emergency_stop_sub_ = node_->create_subscription<std_msgs::msg::Bool>(
        "/planner/emergency_stop", 
        1, 
        std::bind(&MpcController::emergencyStop, this, std::placeholders::_1));

    cmd_time_ = node_->create_wall_timer(
        std::chrono::duration<double>(1.0 / cmd_timer_rate_),
        std::bind(&MpcController::CmdCallback, this));
    ...
    traj_sub_ = node_->create_subscription<carstatemsgs::msg::Polynome>(
        "traj", 
        1, 
        [this](const carstatemsgs::msg::Polynome::SharedPtr msg) {
            TrajCallback(msg);
        }
    );
    
    odom_sub_ = node_->create_subscription<nav_msgs::msg::Odometry>("odom", 1,
        [this](const nav_msgs::msg::Odometry::SharedPtr msg){
        OdomCallback(msg);
    });

    sequence_pub_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>("/mpc/sequence",1);
    ...
    cmd_pub_ = node_->create_publisher<carstatemsgs::msg::CarState>("cmd", 1);
    Ref_path_pub_ = node_->create_publisher<nav_msgs::msg::Path>("/mpc/Ref_path", 10);
    cmd_path_pub_ = node_->create_publisher<nav_msgs::msg::Path>("/mpc/cmd_path", 10);
    Ref_path_marker_pub_ = node_->create_publisher<visualization_msgs::msg::Marker>("/mpc/Ref_path_marker", 10);
    Ref_velocity_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("/mpc/Ref_velocity", 10);
    Real_velocity_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>("/mpc/Real_velocity", 10);
    
    {
        geometry_msgs::msg::PoseStamped ref_v;
...
void MpcController::get_param(){
        // 声明参数（必须先声明才能获取）
    node_->declare_parameter<double>("/mpc/du_threshold", -1.0);
    node_->declare_parameter<double>("/mpc/dt", -1.0);
    node_->declare_parameter<int>("/mpc/max_iter", -1);
    node_->declare_parameter<int>("/mpc/predict_steps", -1);
    node_->declare_parameter<double>("/mpc/max_omega", -1.0);
    node_->declare_parameter<double>("/mpc/max_domega", -1.0);
    node_->declare_parameter<double>("/mpc/max_vel", -1.0);
    node_->declare_parameter<double>("/mpc/min_vel", -1.0);
    node_->declare_parameter<double>("/mpc/max_acc", -1.0);
    node_->declare_parameter<int>("/mpc/delay_num", -1);
    node_->declare_parameter<double>("/mpc/cmd_timer_rate", 100.0);
    node_->declare_parameter<double>("/mpc/max_mpc_time", 10.0);
    node_->declare_parameter<bool>("/mpc/if_mpc", true);
        //nh.param("/mpc/state_seq_res", state_seq_res, 1.0);
    //nh.param("/mpc/Integral_appr_resInt", Integral_appr_resInt, 10);
    node_->declare_parameter<double>("/mpc/state_seq_res", 1.0);
    node_->declare_parameter<int>("/mpc/Integral_appr_resInt", 10);
    // 声明数组参数
    node_->declare_parameter<std::vector<double>>("/mpc/matrix_q", std::vector<double>());
    node_->declare_parameter<std::vector<double>>("/mpc/matrix_r", std::vector<double>());
    node_->declare_parameter<std::vector<double>>("/mpc/matrix_rd", std::vector<double>());
    ...
    node_->get_parameter("/mpc/max_vel", max_speed);
    node_->get_parameter("/mpc/min_vel", min_speed);
    node_->get_parameter("/mpc/max_acc", max_accel);
    node_->get_parameter("/mpc/delay_num", delay_num);
    node_->get_parameter("/mpc/cmd_timer_rate", cmd_timer_rate_);
    node_->get_parameter("/mpc/max_mpc_time", max_mpc_time_);
    node_->get_parameter("/mpc/if_mpc", use_mpc_);
    
    node_->get_parameter("/mpc/matrix_q", Q);
    node_->get_parameter("/mpc/matrix_r", R);
    node_->get_parameter("/mpc/matrix_rd", Rd);

    node_->get_parameter("/mpc/state_seq_res", state_seq_res);
    node_->get_parameter("/mpc/Integral_appr_resInt", Integral_appr_resInt);
}
...
void MpcController::OdomCallback(const nav_msgs::msg::Odometry::SharedPtr msg){    
    ...
    now_state.theta = tf2::getYaw(msg->pose.pose.orientation);
    now_state.v = 0.0;
    now_state.w = 0.0;
...
void MpcController::TrajCallback(const carstatemsgs::msg::Polynome::SharedPtr msg){
    ...
    sequencePub();
    new_traj_start_time_ = rclcpp::Time(msg->traj_start_time).seconds();
...
void MpcController::CmdCallback(){
    if (!has_odom || !receive_traj_)
        return;
    
    if (new_traj_.if_get_traj_ && node_->now().seconds() > new_traj_start_time_){
        traj_ = new_traj_;
        traj_duration = traj_.get_traj_duration();
    ...
    if (at_goal){
        ...
        if(use_mpc_){
            ...
            carstatemsgs::msg::CarState cmd;
            double t_cur = node_->now().seconds() - start_time;
...
{
    int iter;
    Last_plan_time = node_->now().seconds();
    for (iter = 0; iter < max_iter; iter++)
    {
        ...
        if (node_->now().seconds() - Last_plan_time > max_mpc_time_)
        {
            break;
        }
    }
    ...
    for (int i = 0; i < T; i++){
        ...
        q.setRPY(0, 0, xopt[i].theta);  // roll, pitch, yaw
...
void MpcController::emergencyStop(const std_msgs::msg::Bool::SharedPtr msg){
    carstatemsgs::msg::CarState cmd;
    cmd.header.frame_id = "world";
    cmd.header.stamp = node_->now();
    cmd.v = 0.0;
...
void MpcController::getRefPoints(const int T, double dt){
    ...
    geometry_msgs::msg::PoseStamped ref_v;
    auto current_a = traj_.getAstate(std::min(node_->now().seconds() - start_time, traj_duration));
    auto current_v = traj_.getVstate(std::min(node_->now().seconds() - start_time, traj_duration));
    auto current_p = traj_.getPstate(std::min(node_->now().seconds() - start_time, traj_duration));
    ref_v.header.stamp = node_->now();
...
void MpcController::sequencePub(){
    ...
    globalMap_pcd.header.stamp = node_->now();
