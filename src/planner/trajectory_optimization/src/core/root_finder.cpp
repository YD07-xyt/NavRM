...
namespace RootFinderPriv {
    ...
    solve_quart_monic(double a, double b, double c, double d)
    {
        ...
        double x3[3];
        int i_zeroes = solve_resolvent(x3, a3, b3, c3);
    ...
    // All coefficients can be zero
    {
        if (fabs(a) < DBL_EPSILON) {
            ...
            return solve_quart_monic(b / a, c / a, d / a, e / a);
        }
    }

    inline std::set<double> eigen_solve_real_roots(
    ...
    inline double
    num_sign_var(double x, double** sturmSeqs, int* szSeq, int len)
    {
        double y, lasty;
        int signVar = 0;
        lasty = poly_eval(sturmSeqs[0], szSeq[0], x);
        for (int i = 1; i < len; i++) {
            y = poly_eval(sturmSeqs[i], szSeq[i], x);
        ...
        double tol)
    {
        ...
        poly_deri(coeffs, dcoeffs, numCoeffs);
        auto func = [&coeffs, &numCoeffs](double x) {
            return poly_eval(coeffs, numCoeffs, x);
        };
        auto dfunc = [&dcoeffs, &numCoeffs](double x) {
            return poly_eval(dcoeffs, numCoeffs - 1, x);
        };
        constexpr int maxDblIts = 128;
        double rts = safe_newton(func, dfunc, lbound, ubound, tol, maxDblIts);
        delete[] dcoeffs;
        return rts;
    }
        ...
        std::set<double>& rts)
    {
        ...
        if (nrts == 0) {
            ...
            if (fl * fr < 0) {
                rts.insert(shrink_interval(sturmSeqs[0], szSeq[0], l, r, tol));
                return;
            }
            else {
                ...
                for (int i = 0; i < maxDblIts; i++) {
                    ...
                    if (fl * fr < 0) {
                        rts.insert(
                            shrink_interval(sturmSeqs[1], szSeq[1], l, r, tol));
                        return;
                    }

                    m = (l + r) / 2.0;
                    fm = poly_eval(sturmSeqs[0], szSeq[0], m);
                    ...
                    if (fm == 0 || fabs(r - l) < tol) {
                        ...
                        if (lnv == num_sign_var(m, sturmSeqs, szSeq, len)) {
                            l = m;
                            fl = fm;
                        }
                        else {
            ...
            for (int i = 0; i < maxDblIts; i++) {
                ...
                mnv = num_sign_var(m, sturmSeqs, szSeq, len);
    ...
    //              coeffs(lbound) != 0, coeffs(rbound) != 0, lbound < rbound
    {
        ...
        double sturmSeqs[(RootFinderParam::HIGHEST_ORDER + 1) *
                         (RootFinderParam::HIGHEST_ORDER + 1)];
        int szSeq[RootFinderParam::HIGHEST_ORDER + 1] = {
            0};// Explicit ini as zero (gcc may neglect this in -O3)
        double* offsetSeq[RootFinderParam::HIGHEST_ORDER + 1];
        int num = 0;
        ...
        while (!remainderConstant) {
            szSeq[idx + 2] = poly_mod(offsetSeq[idx],
                offsetSeq[idx + 1],
        ...
        recur_isolate(lbound,
            ubound,
            poly_eval(offsetSeq[0], szSeq[0], lbound),
            poly_eval(offsetSeq[0], szSeq[0], ubound),
            num_sign_var(lbound, offsetSeq, szSeq, len),
            num_sign_var(ubound, offsetSeq, szSeq, len),
            tol,
            offsetSeq,
            szSeq,
            len,
...
namespace RootFinder {
    ...
    // Calculate self-convolution of coef(x)
    {
        ...
        for (int i = 0; i < resultSize; i++) {
            ...
            rbound += lbound;
            if ((rbound & 1) != 0)//faster than rbound % 2 == 1
            {
    ...
    // Boundary values, i.e., coeffs(l) and coeffs(r), must be nonzero
    {
        ...
        if (valid > 0 && fabs(coeffs(originalSize - 1)) > DBL_EPSILON) {
            ...
            int idx = 0;
            while (!remainderConstant) {
                szSeq[idx + 2] = RootFinderPriv::poly_mod(
            ...
            lastyl = RootFinderPriv::poly_eval(&(sturmSeqs[len - szSeq[0]]),
                szSeq[0],
                l);
            lastyr = RootFinderPriv::poly_eval(&(sturmSeqs[len - szSeq[0]]),
                szSeq[0],
                r);
            for (int i = 1; i < num; i++) {
                yl = RootFinderPriv::poly_eval(
                    &(sturmSeqs[(i + 1) * len - szSeq[i]]),
                    szSeq[i],
                    l);
                yr = RootFinderPriv::poly_eval(
                    &(sturmSeqs[(i + 1) * len - szSeq[i]]),
                    szSeq[i],
                    r);
        ...
        bool isolation)
    {
        ...
        if (nonzeros == 0) {
            ...
            if (nonzeros <= 5) {
                rts = RootFinderPriv::solve_quart(ncoeffs(0),
                    ncoeffs(1),
                    ncoeffs(2),
                    ncoeffs(3),
                    ncoeffs(4));
                ...
                if (isolation) {
                    rts = RootFinderPriv::isolate_real_roots(ncoeffs,
                        lbound,
                        ubound,
                        tol);
                }
                else {
                    rts = RootFinderPriv::eigen_solve_real_roots(ncoeffs,
                        lbound,
                        ubound,
                        tol);
                }
