...
JPSPlanner::JPSPlanner(std::shared_ptr<planner::map::Map> map,JPSPlannerParams params): map_util_(map){
    ...
    safe_dis_=params.safe_dis_;
    max_jps_dis_=params.max_jps_dis_;
    distance_weight_=params.distance_weight_;
    yaw_weight_=params.yaw_weight_;
    trajCutLength_=params.trajCutLength_;
    max_vel_=params.max_vel_;
    max_acc_=params.max_acc_;
    max_omega_=params.max_omega_;
    max_domega_=params.max_domega_;
    sampletime_=params.sampletime_;
    mintrajNum_=params.mintrajNum_;
...
void JPSPlanner::get_small_resolution_path_(){
    ...
    for(int i = 0; i < path_size - 1; i++){
        Eigen::Vector2i start = map_util_->occupancy_grid_map.coord2gridIndex(path_[i]);
        Eigen::Vector2i end = map_util_->occupancy_grid_map.coord2gridIndex(path_[i+1]);
...
bool JPSPlanner::JPS_check_if_collision(const Eigen::Vector2d &pos){
  return map_util_->occupancy_grid_map.getDistanceReal(pos) > safe_dis_;
}
}